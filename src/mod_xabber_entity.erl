...
%% API
-export([is_exist/2, is_exist_anywhere/2]).
...
is_exist(LUser, LServer, true) ->
  case ejabberd_sql:sql_query(
    LServer,
    ?SQL("select @(localpart)s from
    (
    (select @(localpart)s,@(server_host)s from groupchats)
    UNION
    (select @(localpart)s,@(server_host)s from channels)
    ) as t
...
case ejabberd_sql:sql_query(
  ...
  ?SQL("select @(localpart)s from
    (
    (select @(localpart)s from groupchats)
    UNION
    (select @(localpart)s from channels)
    ) as t
    where localpart = %(LUser)s and %(LServer)H")) of
  {selected,[{null}]} ->
    false;
