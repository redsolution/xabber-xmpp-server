...
%% API
-export([init/2, lookup_session/4, store_session/9,
...
store_session(_LUser, _LServer, _TS, _PushJID, _Node, _XData, _EncryptionType, _EncriptionKey, undefined) ->
	{error, db_failure};
store_session(LUser, LServer, TS, PushJID, Node, XData, EncryptionType, EncriptionKey, DeviceID) ->
	US = {LUser, LServer},
	PushLJID = jid:tolower(PushJID),
	EncryptionKey = base64:encode(EncriptionKey),
	F = fun() ->
			delete_conflicting_sessions(US, TS, PushLJID, Node, DeviceID),
		...
		case mnesia:transaction(F) of
			{atomic, ok} ->
				{ok, {TS, PushLJID, Node, XData, EncryptionType, EncryptionKey}};
		{aborted, E} ->
...
delete_conflicting_sessions(US, TS, PushLJID, Node, DeviceID) ->
	...
	lists:foreach(
	  fun(Rec) ->
		  case conflicts_with(Rec, TS, PushLJID, Node, DeviceID) of
		      true -> mnesia:delete_object(Rec);
		      false -> ok
		  end
	  end, Recs).

conflicts_with(#xabber_push_session{timestamp = TS}, TS, _PushLJID, _Node, _DeviceID) ->
	true;
conflicts_with(#xabber_push_session{service = PushLJID, node = Node}, _TS, PushLJID, Node, _DeviceID) ->
	true;
conflicts_with(#xabber_push_session{device_id = DeviceID}, _TS, _PushLJID, _Node, DeviceID)
  when DeviceID /= undefined ->
	true;
conflicts_with(_Rec, _TS, _PushLJID, _Node, _DeviceID) ->
	false.
