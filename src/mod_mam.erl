...
start(Host, Opts) ->
    ...
    ejabberd_hooks:add(user_send_packet, Host, ?MODULE,
		       user_send_packet, 88),
    case gen_mod:get_opt(strip_outgoing_stanza_id, Opts) of
	true ->
	    ejabberd_hooks:add(user_send_packet, Host, ?MODULE,
...
stop(Host) ->
    ...
    ejabberd_hooks:delete(user_send_packet, Host, ?MODULE,
			  user_send_packet, 88),
    case gen_mod:get_module_opt(Host, ?MODULE, strip_outgoing_stanza_id) of
	true ->
	    ejabberd_hooks:delete(user_send_packet, Host, ?MODULE,
				  user_send_packet_strip_tag, 500);
	false ->
...
user_receive_packet({#message{from = Peer} = Pkt, #{jid := JID} = C2SState}) ->
    ...
    Pkt1 = case should_archive_in(Pkt, LServer) of
	       true ->
		   case store_msg(Pkt, Pkt, LUser, LServer, Peer, recv) of
		       {ok, StoredPkt} ->
			   case ejabberd_hooks:run_fold(
                                   receive_message_stored, LServer,
                                   {ok, StoredPkt}, []) of
                               {ok, NewStoredPkt} ->
                                   mark_stored_msg(NewStoredPkt, JID);
                               _ ->
                                   StoredPkt
                           end;
		       _ ->
...
user_send_packet({#message{to = Peer} = OPkt, #{jid := JID} = C2SState}) ->
    ...
    Pkt2 = case should_archive_out(Pkt1, LServer) of
		   ...
		   case store_msg(Pkt1, xmpp:set_from_to(Pkt1, JID, Peer),
		       ...
		       {ok, StoredPkt} ->
			   StoredID = get_stanza_id(StoredPkt),
			   case ejabberd_hooks:run_fold(
                                   sent_message_stored, LServer,
                                   {ok, StoredPkt}, [LServer, JID, StoredID, Request]) of
                               {ok, NewStoredPkt} ->
                                   mark_stored_msg(NewStoredPkt, JID);
                               _ ->
                                   StoredPkt
...
offline_message({_Action, #message{from = Peer, to = To} = Pkt} = Acc) ->
    ...
    case should_archive_in(Pkt, LServer) of
       true ->
	   case store_msg(Pkt, Pkt, LUser, LServer, Peer, recv) of
               {ok, StoredPkt} ->
		   {archived, mark_stored_msg(StoredPkt, To)};
	       _ ->
		   Acc
	   end;
       false ->
...
muc_filter_message(#message{from = From} = Pkt,
    ...
    if Config#config.mam ->
	    ...
	    case store_muc(Pkt1, MUCState, StorePkt,
                    RoomJID, From, FromNick) of
                {ok, StoredPkt} ->
		    mark_stored_msg(StoredPkt, RoomJID);
		_ ->
		    Pkt1
	    end;
	true ->
...
-spec set_stanza_id(stanza(), jid(), binary(), erlang:timestamp()) -> stanza().
set_stanza_id(Pkt, JID, ID, TimeStamp) ->
...
process_iq(LServer, IQ, MsgType) ->
  spawn(async_process_iq(LServer, IQ, MsgType)),
  ignore.

async_process_iq(LServer, IQ, MsgType) ->
  fun() ->
    case do_process_iq(LServer, IQ, MsgType) of
      ignore -> ok;
      Result ->
...
do_process_iq(LServer, #iq{from = #jid{luser = LUser}, lang = Lang,
...
-spec should_archive_out(message(), binary()) -> boolean().
should_archive_out(#message{type = error}, _LServer) ->
	false;
should_archive_out(#message{type = groupchat}, _LServer) ->
	false;
should_archive_out(#message{meta = #{from_offline := true}}, _LServer) ->
	false;
should_archive_out(#message{from = From, body = Body, subject = Subject,
...
-spec should_archive_in(message(), binary()) -> boolean().
should_archive_in(#message{type = error}, _LServer) ->
	false;
should_archive_in(#message{type = groupchat}, _LServer) ->
	false;
...
-spec strip_my_stanza_id_new_incoming(stanza(), jid()) -> stanza().
strip_my_stanza_id_new_incoming(Pkt, ByJID) ->
	Els = xmpp:get_els(Pkt),
	NewEls = lists:filter(
		fun(El) ->
			Name = xmpp:get_name(El),
			NS = xmpp:get_ns(El),
      LServer = ByJID#jid.lserver,
			if (Name == <<"archived">> andalso NS == ?NS_MAM_TMP);
			(Name == <<"time">> andalso NS == ?NS_UNIQUE);
			(Name == <<"stanza-id">> andalso NS == ?NS_SID_0) ->
				try xmpp:decode(El) of
...
-spec is_archived_by(message(), jid()) -> boolean().
is_archived_by(Pkt, By) ->
	case xmpp:get_subtag(Pkt, #stanza_id{by = #jid{}}) of
		#stanza_id{by = By} ->
			true;
		_ ->
...
-spec store_muc(message(), mod_muc_room:state(), message(),
        jid(), jid(), binary()) -> {ok, message()} | pass | any().
store_muc(OriginPkt, MUCState, Pkt, RoomJID, Peer, Nick) ->
...
msg_to_el(#archive_msg{timestamp = TS, packet = El, nick = Nick,
	...
	Pkt1 ->
	    Pkt2 = set_stanza_id(Pkt1, JidArchive, ID, TS),
		...
		_ ->
			...
			if NS /= ?NS_MAM_0 ->
				...
				true ->
					...
					ejabberd_router:route(
						#message{from = From, to = To, sub_els = [Result, Hint]}),
					ignore
			end
	end.
...
-spec limit_max(rsm_set() | undefined, binary()) -> rsm_set().
...
mod_opt_type(strip_outgoing_stanza_id) ->
    fun (B) when is_boolean(B) -> B end;
