...
process_local_iq(#iq{type = get, to = #jid{lserver = LServer}, lang = Lang} = IQ) ->
    Desc = translate:translate(Lang, <<"Xabber Server">>),
		Company = <<"Redsolution OU">>,
    xmpp:make_iq_result(
      IQ, #vcard_temp{fn = <<LServer/binary," server">>,
		      url = <<"https://www.redsolution.com/">>,
		      desc = <<Desc/binary, $\n, Company/binary>>,
		      bday = <<"2002-11-16">>}).
...
get_vcard(LUser, LServer) ->
	case mod_xabber_entity:get_entity_type(LUser,LServer) of
		group -> groups_avatars:get_vcard(LUser,LServer);
		_ ->
